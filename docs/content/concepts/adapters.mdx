...
Pilot ships with 10 adapters across three categories: issue trackers, chat platforms, and project boards.
...
| **Discord** | ❌ | ✅ | — | — | Bot command |
| **Signal** | ❌ | WS receive | UUID | — | Message in an allowed group |
...
**Signal** is the private-messenger adapter, reached through signal-cli-rest-api in `json-rpc` mode. Signal has no per-channel permission model, so membership of an allowlisted group *is* the authorization boundary — an empty `groups` list admits nothing rather than everything. Confirmations and approvals are native Signal polls, restricted to the UUIDs in `approvers` when that list is set.
