...
type Engine struct {
	...
	activeAlerts        map[string]*activeAlert // rule|source -> alert awaiting recovery
...
// Channels are resolved at fire time so the resolution reaches the same recipients.
...
// Bypasses cooldown; reuses the original alert's channels, not its own severity.
...
// Inline before Start() so direct callers and tests stay synchronous (E1).
