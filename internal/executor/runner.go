...
Only use DECLINED if implementation is truly impossible or undefined. Do not decline due to difficulty alone.`, task.Title, task.Description)
		...
		// ReadOnly skips gates, so a ReadOnly task that also ships a PR would go out ungated.
		if task.ReadOnly && task.CreatePR {
			log.Warn("task sets both ReadOnly and CreatePR — quality gates are skipped for ReadOnly tasks, so this PR ships ungated",
				slog.String("task_id", task.ID),
			)
		}
